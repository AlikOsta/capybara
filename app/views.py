...
def product_list(request):
    ...
    total = base_qs.count()
    paginator = Paginator(base_qs.order_by('-created_at'), 32)
...
class ProductListView(PublishedMixin, SearchMixin, ListView):
    model = Product
    ...
    def get_queryset(self):
        qs = Product.objects.filter(status=3)
...
# API Views reuse common filtering
def get_base_qs(request):
    qs = Product.objects.filter(status=3)
    qs = qs.select_related('author','category','currency','city')
    return qs
...
class ProductListAPIView(View):
    def get(self, request, *args, **kwargs):
        ...
        qs = get_base_qs(request)
        qs = SearchMixin().filter_search(qs)
        total = qs.count()
        items = qs.order_by('-created_at')[offset:offset+limit]
        fav_ids = list(request.user.favorites.values_list('product_id', flat=True)) if request.user.is_authenticated else []
...
class CategoryProductsAPIView(View):
    def get(self, request, *args, **kwargs):
        cat = get_object_or_404(Category, slug=kwargs.get('category_slug'))
        offset, limit = map(int, (request.GET.get('offset',0), request.GET.get('limit',10)))
        qs = get_base_qs(request).filter(category=cat)
        qs = SearchMixin().filter_search(qs)
